...
**Start Channel Hoping:** Great, Script Started to Provide captured Beacons Frames to us. There are many wirless channels and changing from one channel to another is called Channel Hopping. By Default Scapy don't Support Channel Hoping So, until now we only captured the Beacon Frame of Only One Wireless Channel.At the moment our NIC card is in the Monitor Mode now,lets find our new Wireless interface. Now, Open another terminal (ctrl + shift + N ) name again by typing **iwconfig** now my Wireless interface is wlp3s0mon. Now Open another terminal (ctrl + shift + N ) and in new terminal type *airodump-ng* <**Wireless Interface name** +mon>  example: for me *sudo airmon-ng start wlp3s0mon* . Now, Channel Hopping started and the script is now providing the Beacon Frames of all Wireless Channel.      
